        ...
        for (const stepIndex of scenario.dag.steps.map(s => s.index)) {
            ...
            if (useGemini) {
                try {
                    ...
                    this.addLog(`Gemini API failed (quota?), using mock adapter: ${e.message.slice(0, 50)}`);
            ...
            // Submit to Midnight
            await client.submitAttestation(taskResult.taskId, stepIndex, signature, `hash_${stepIndex}` // In practice this would be actual SHA256 of result
            );
