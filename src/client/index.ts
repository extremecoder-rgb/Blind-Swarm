...
function signTransactionIntents(tx: any, signFn: any, proofMarker: 'proof' | 'pre-proof' | 'no-proof'): void {
...
export class MidnightClient {
  ...
  async initWallet(): Promise<void> {
    ...
    this.wallet = await WalletFacade.init({
      ...
      shielded: (config: any) => ShieldedWallet(config).startWithSecretKeys(this.shieldedSecretKeys),
      ...
      dust: (config: any) => DustWallet(config).startWithSecretKey(this.dustSecretKey, ledger.LedgerParameters.initialParameters().dust),
    });
    ...
    const state: any = await Rx.firstValueFrom(this.wallet.state().pipe(Rx.take(1)));
    ...
    if (state.dust.availableCoins.length === 0) {
      ...
      if (nightUtxos.length > 0) {
        ...
        const recipe = await this.wallet.registerNightUtxosForDustGeneration(
          nightUtxos, this.unshieldedKeystore.getPublicKey(), (p: Uint8Array) => this.unshieldedKeystore.signData(p),
        );
    ...
    const zkConfigProvider: any = new NodeZkConfigProvider(zkConfigPath);
  ...
  private async createWalletAndMidnightProvider(state: any) {
    return {
  ...
  async getBalance(): Promise<{ tNight: bigint; dust: bigint }> {
    if (!this.wallet) throw new Error('Wallet not initialized');
    const state: any = await Rx.firstValueFrom(this.wallet.state().pipe(Rx.take(1)));
    return {
  ...
  async deployContract(): Promise<DeployResult> {
    if (!this.providers) throw new Error('Client not initialized');
    ...
    const compiledContract = (CompiledContract as any).make('BlindSwarm', contractModule.Contract).pipe(
    ...
    try {
      const deployed: any = await deployContract(this.providers, {
        compiledContract,
  ...
  async submitAttestation(taskId: string, stepBytes: Uint8Array, outputHash: string): Promise<{ success: boolean }> {
